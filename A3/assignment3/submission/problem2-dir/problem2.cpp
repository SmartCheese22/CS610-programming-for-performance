...
int avx2_version(const int* __restrict__ source, int* __restrict__ dest) {
  ...
  const int stride = (AVX2_WIDTH_BITS / (sizeof(int) * CHAR_BIT)); // 8 ints per 256-bit vector

  for (int i = 0; i < N; i += stride) {
    __m256i x = _mm256_load_si256((__m256i*)&source[i]);

    // per-128-bit prefix
    __m256i t0 = _mm256_slli_si256(x, 4);
    __m256i t1 = _mm256_add_epi32(x, t0);
    __m256i t2 = _mm256_slli_si256(t1, 8);
    __m256i ps = _mm256_add_epi32(t1, t2);

    // extract carry (last element of lower lane)
    __m128i ps_lo = _mm256_castsi256_si128(ps);
    int carry = _mm_extract_epi32(ps_lo, 3);

    // add carry to upper lane only
    __m256i add_hi = _mm256_setr_epi32(0,0,0,0,carry,carry,carry,carry);
    ps = _mm256_add_epi32(ps, add_hi);

    // add offset from previous block
    ps = _mm256_add_epi32(ps, offset);

    // store
    _mm256_store_si256((__m256i*)&dest[i], ps);

    // compute new offset (last element of upper lane)
    __m128i ps_hi = _mm256_extracti128_si256(ps, 1);
    int new_offset = _mm_extract_epi32(ps_hi, 3);
    offset = _mm256_set1_epi32(new_offset);
  }
